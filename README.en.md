...
> [!IMPORTANT]
> `v0.6.6` does four things. **Half of all transferred bytes were duplicates; now none are**: a completed chunk has to be written to disk first, and for the whole duration of that round trip it counted as neither in-flight nor received — so the scheduler decided it was still missing and asked for it again. Measured on a 25.9 MB file, the sender pushed out 59.8 MB (2.31×); the extra copy was eventually recognized as a duplicate and discarded, but the bandwidth was already spent. **Invite codes now appear in 1.3 s instead of 9 s**: serverless mode waits for ICE candidate gathering, and `iceGatheringState` never reaches `complete` under Electron, so every code burned the full 8-second timeout. Gathering now stops 1.2 s after the last new candidate once a public address is known — with no candidates lost. **Connections establish more often**: STUN redundancy went from one server to three, TURN addresses are tried over both UDP and TCP (hotel and corporate networks frequently allow only TCP), a dropped link reconnects itself instead of forcing everyone to rejoin, and a failure now says whether STUN is unreachable, the relay credentials are wrong, or both sides are behind symmetric NAT. **The lossless slim-down now actually compresses**: uncompressed PCM audio is converted to FLAC, decoding byte-for-byte identically. The compression ratio is measured on your actual file rather than assumed, and the conversion is not offered when it would save less than 8%. You can also choose which audio track to keep before sharing.

## Downloads

| Build | Best for | Download |
|---|---|---|
| Windows full installer | Recommended. Bundles mpv and yt-dlp and lets you choose the install folder | [NoxReel-Setup-0.6.6.exe](https://github.com/Felis-desuwa/NoxReel/releases/latest/download/NoxReel-Setup-0.6.6.exe) |
| Windows web installer | Smaller guided installer with a selectable folder; downloads components during setup | [NoxReel-WebSetup-0.6.6.exe](https://github.com/Felis-desuwa/NoxReel/releases/latest/download/NoxReel-WebSetup-0.6.6.exe) |
