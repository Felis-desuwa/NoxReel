...
> [!IMPORTANT]
> `v0.6.7` is a bug-fix release. A full-repository audit (17 dimensions searching independently, every finding then attacked by three verifiers from different angles) confirmed **47 defects**; all are fixed, and about ten of them were introduced by `v0.6.6` itself. **The most serious one bypassed Safe mode**: the room’s "Reopen player" button called the launch function directly, while the scan gate lived only in the other callers — so the button appeared as soon as a receiving session existed, and one click handed a partially received, never-scanned file to the player. The gate now lives inside the launch function, where a forgetful caller cannot get around it. **The lossless slim-down failed outright on most files**: with no PCM track eligible for FLAC, the renderer sent an empty array, and the main process rejected an empty array as malformed input — meaning the feature only worked when the source happened to carry uncompressed PCM. **Sampled bitrate estimation had never once worked**: ffprobe does not emit fields in the requested order, so the code read packet duration as packet size and the function always came back empty. **Automatic reconnection collided with itself**: when both sides recovered at once, the offering side both ran its own timer and answered the peer’s request, sending two offers; the stale answer landed on the new connection and the link could never come up. Android did not implement the protocol at all, so a dropped phone link never recovered. Also fixed: an in-flight accounting leak that permanently evicted an upstream and froze the progress bar, pressing space during a room-wide pause permanently releasing your own pause, reopening the player dragging the whole room back to 00:00, XFF spoofing and host-identity takeover on the signaling server, Android receive caches never being deleted, and a batch of untranslated interface strings. Tests grew from 205 to 252.

## Downloads

| Build | Best for | Download |
|---|---|---|
| Windows full installer | Recommended. Bundles mpv and yt-dlp and lets you choose the install folder | [NoxReel-Setup-0.6.7.exe](https://github.com/Felis-desuwa/NoxReel/releases/latest/download/NoxReel-Setup-0.6.7.exe) |
| Windows web installer | Smaller guided installer with a selectable folder; downloads components during setup | [NoxReel-WebSetup-0.6.7.exe](https://github.com/Felis-desuwa/NoxReel/releases/latest/download/NoxReel-WebSetup-0.6.7.exe) |
